 ...
 */
export async function toggleConcurrentExemption(
  loanId: string,
  allowed: boolean,
) {
...
export const submitLoanApproval = withAudit(
  ...
  async (loanId: string, decision: "APPROVED" | "REJECTED", notes: string) => {
    ...
    const roleHasPermission = [
      ...
      "TREASURER",
      "SYSTEM_ADMINISTRATOR",
    ].includes(userRole); // Quick check aligned with approval-actions
    ...
    if (!roleHasPermission && !hasGranularPermission) {
      ...
      const hasValidDelegation = activeDelegations.some((d) => {
        ...
        return [
          ...
          "TREASURER",
          "SYSTEM_ADMINISTRATOR",
        ].includes(delegatorRole);
        // Note: We could also check granular permissions of delegator here if needed
      });

      if (!hasValidDelegation) {
        throw new Error(
          "Unauthorized: You do not have permission (direct or delegated) to approve loans",
        );
      }
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (loan.memberId === approverId) {
        throw new Error(
          "Compliance Error: You cannot approve your own loan application",
        );
      }
      ...
      AuditContext.log("Quorum Status", {
        approvedCount,
        requiredApprovals,
        hasExecutiveApproval,
      });
      ...
      if (approvedCount >= requiredApprovals && hasExecutiveApproval) {
        ...
        return {
          ...
          message: "Quorum met, waiting for Executive Approval",
        };
      }

      return {
        status: "PENDING_QUORUM",
        message: `Vote recorded. (${approvedCount}/${requiredApprovals})`,
      };
    });
...
export async function disburseLoanToWallet(loanId: string) {
  return withAudit(AuditLogAction.LOAN_DISBURSED, async (_: string) => {
    // _ because we use the outer loanId, or keep it as loanId but it shadows
    const session = await auth();
    if (!session?.user) {
    ...
    return await prisma.$transaction(
      async (tx) => {
        ...
        // Check authorization: Role-based, Permission-based, OR Loan Owner
        const userRole = session!.user.role;
        ...
        if (userPermissions) {
          if (Array.isArray(userPermissions)) {
            hasGranularPermission =
              userPermissions.includes("DISBURSE_LOANS") ||
              userPermissions.includes("ALL");
          } else if (typeof userPermissions === "object") {
            // @ts-ignore
            hasGranularPermission =
        ...
        if (!hasRolePermission && !hasGranularPermission && !isOwner) {
          throw new Error(
            "Unauthorized: You do not have permission to disburse loans. Required permission: DISBURSE_LOANS or be the loan applicant.",
          );
        }

        AuditContext.track("Permission Checked", {
        ...
        if (loan.status !== "APPROVED") {
          throw new Error(
            `Loan cannot be disbursed. Current status: ${loan.status}. Only APPROVED loans can be disbursed.`,
          );
        }
        ...
        const dPrincipal = new Prisma.Decimal(loan.amount || 0).toDecimalPlaces(
          2,
        );
        const dProcessingFee = new Prisma.Decimal(
          loan.processingFee || 0,
        ).toDecimalPlaces(2);
        const dInsuranceFee = new Prisma.Decimal(
          loan.insuranceFee || 0,
        ).toDecimalPlaces(2);
        const dShareDeduction = new Prisma.Decimal(
        ...
        const getCode = (type: SystemAccountType) =>
          mappings[type] || DEFAULT_MAPPINGS[type];
        ...
        const getAccountId = async (
          ...
          if (!acc)
            throw new Error(
              `Account with code ${code} not found. Ensure system accounting is configured.`,
            );
          return acc.id;
        };

        // 2. Resolve Member Wallet
        const memberWallet = await WalletService.createWallet(
          loan.memberId,
          tx,
        );
        ...
        if (dProcessingFee.gt(0)) {
          const accId = await getAccountId(
            tx,
            getCode("INCOME_LOAN_PROCESSING_FEE"),
          );
          finalJournalLines.push({
            accountId: accId,
            accountType: "INCOME",
        ...
        for (const topUp of loan.topUps) {
          const dTopUpTotal = new Prisma.Decimal(
            topUp.totalOffset,
          ).toDecimalPlaces(2);
          const dRefinanceFee = new Prisma.Decimal(
            topUp.refinanceFee || 0,
          ).toDecimalPlaces(2);
        ...
        AuditContext.track("Journal Entry Posted", {
          jeId: je.id,
          totalAmount: dPrincipal.toNumber(),
        });
        ...
        const strictBalance = await LoanBalanceService.updateLoanBalance(
          loanId,
          tx,
        );
        ...
        for (const topUp of loan.topUps) {
          ...
          if (dClearance.gt(0)) {
            ...
            const oldLoanBalance = await LoanBalanceService.updateLoanBalance(
              topUp.oldLoanId,
              tx,
            );
