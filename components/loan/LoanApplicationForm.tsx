...
import { Member, LoanProduct, Loan, LoanQualification, ActiveLoanSummary } from '@/lib/types';
...
export function LoanApplicationForm({
    ...
    const [activeLoans, setActiveLoans] = useState<ActiveLoanSummary[]>([]);

    const [qualification, setQualification] = useState<LoanQualification | null>(null);
    ...
    const netDisbursement = useMemo(() => {
        const principal = watchedAmount ? parseFloat(watchedAmount) : 0;
        if (isNaN(principal) || principal <= 0) return 0;
        return principal - totalDeductions;
    }, [watchedAmount, totalDeductions]);
    ...
    async function handleFormAction(formData: FormData) {
        ...
        await execute(async (idempotencyKey) => {
            ...
            if (intent === 'send') {
                ...
                let res: any;
                try {
                    res = await applyForLoan(null, formData);
                } finally {
                    clearInterval(interval);    // always runs — no leaked interval
                }
            ...
            try {
                ...
                toast.error('Could not save draft. Please check your connection.');
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="bg-gradient-to-br from-purple-50 to-purple-100 border-2 border-purple-200 p-8 rounded-2xl shadow-lg text-center">
                                                ...
                                                <div className="text-5xl font-black text-purple-700 mb-2">
                                                    KES {netDisbursement.toLocaleString('en-KE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                </div>
